...
void PaintFrame::OnExport(wxCommandEvent& event)
{
    wxFileDialog saveFileDialog(this, "Save File", "", "",
                                "BMP files (*.bmp)|*.bmp|PNG files (*.png)|*.png|JPG and JPEG files (*.jpg;*.jpeg)|*.jpg;*jpeg", wxFD_SAVE | wxFD_OVERWRITE_PROMPT);
    ...
    else
    {
        ...
        mModel->Export(filename, mPanel->GetSize());
    }
}

void PaintFrame::OnImport(wxCommandEvent& event)
{
    wxFileDialog openFileDialog(this, "Load File", "", "",
                            "BMP files (*.bmp)|*.bmp|PNG files (*.png)|*.png|JPG and JPEG files (*.jpg;*.jpeg)|*.jpg;*jpeg", wxFD_OPEN | wxFD_FILE_MUST_EXIST);
...
void PaintFrame::OnSetPenWidth(wxCommandEvent& event)
{
    ...
    {
        wxString entryString = textEntry.GetValue();
        if (!entryString.empty())
        {
            int width = wxAtoi(entryString);
            if (width < 11 && width > 0)
            {
    ...
    if (event.LeftDown())
    {
        ...
        switch (mCurrentTool)
        {
            ...
            case ID_DrawPencil:
                ...
                break;
            case ID_Selector:
                mModel->Intersect(event.GetPosition());
                break;
        }
    }
...
void PaintFrame::EnableUndoRedo()
{
...
void PaintFrame::EnableUnSelect()
{
    mEditMenu->Enable(ID_Unselect, mModel->HasSelection());
}

void PaintFrame::EnableDelete()
{
